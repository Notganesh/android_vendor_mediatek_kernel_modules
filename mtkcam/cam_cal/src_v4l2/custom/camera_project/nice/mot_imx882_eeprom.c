...
static unsigned int mot_imx882_do_pdaf(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
#define IMX882_MOT_EEPROM_ADDR 0x00
#define IMX882_MOT_EEPROM_DATA_SIZE 0x3284
#define IMX882_MOT_SERIAL_NUMBER_ADDR 0x15
#define IMX882_MOT_MNF_ADDR 0x00
#define IMX882_MOT_MNF_DATA_SIZE 37
#define IMX882_MOT_MNF_CHECKSUM_ADDR 0x25
#define IMX882_MOT_AF_ADDR 0x27
#define IMX882_MOT_AF_DATA_SIZE 24
#define IMX882_MOT_AF_CHECKSUM_ADDR 0x3F
#define IMX882_MOT_AWB_ADDR 0x41
#define IMX882_MOT_AWB_DATA_SIZE 43
#define IMX882_MOT_AWB_CHECKSUM_ADDR 0x6C
#define IMX882_MOT_LSC_ADDR 0xC96
#define IMX882_MOT_LSC_DATA_SIZE 1868
#define IMX882_MOT_LSC_CHECKSUM_ADDR 0x13E2
#define IMX882_MOT_PDAF1_ADDR 0x13E4
#define IMX882_MOT_PDAF2_ADDR 0x15D4
#define IMX882_MOT_PDAF_DATA1_SIZE 496
#define IMX882_MOT_PDAF_DATA2_SIZE 1004
#define IMX882_MOT_PDAF_CHECKSUM_ADDR 0x19C0
#define IMX882_MOT_AF_SYNC_DATA_ADDR 0x19C4
#define IMX882_MOT_AF_SYNC_DATA_SIZE 24
#define IMX882_MOT_AF_SYNC_DATA_CHECKSUM_ADDR 0x19DC

#define IMX882_MOT_MTK_NECESSARY_DATA_ADDR 0x19DE
#define IMX882_MOT_MTK_NECESSARY_DATA_SIZE 18
#define IMX882_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR 0x19F0

#define IMX882_QSC_DATA_ADDR 0x1A71
#define IMX882_QSC_DATA_SIZE 3072
#define IMX882_QSC_CRC_ADDR 0x2671

#define IMX882_SPC_DATA_ADDR 0x2673
#define IMX882_SPC_DATA_SIZE 384
#define IMX882_SPC_CRC_ADDR 0x27F3

#define IMX882_FULL_PD_PROC1_ADDR 0x13E4
#define IMX882_FULL_PD_PROC1_SIZE 496
#define IMX882_FULL_PD_PROC2_ADDR 0x15D4
#define IMX882_FULL_PD_PROC2_SIZE 1004
#define IMX882_FULL_PD_SIZE (IMX882_FULL_PD_PROC1_SIZE+IMX882_FULL_PD_PROC2_SIZE)

#define IMX882_PARTIAL_PD_PROC1_ADDR 0x2CA4
#define IMX882_PARTIAL_PD_PROC1_SIZE 496
#define IMX882_PARTIAL_PD_PROC2_ADDR 0x2E94
#define IMX882_PARTIAL_PD_PROC2_SIZE 1004
#define IMX882_PARTIAL_PD_SIZE (IMX882_PARTIAL_PD_PROC1_SIZE+IMX882_PARTIAL_PD_PROC2_SIZE)
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000003, 0x32443832, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, IMX882_MOT_LSC_ADDR, IMX882_MOT_LSC_DATA_SIZE, mot_do_single_lsc},//CAMERA_CAM_CAL_DATA_SHADING_TABLE
		{0x00000001, 0x00000027, 0x00000045, mot_imx882_do_2a_gain},//CAMERA_CAM_CAL_DATA_3A_GAIN
		{0x00000001, 0x00000000, 0x00000000, mot_imx882_do_pdaf},//CAMERA_CAM_CAL_DATA_PDAF
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT mot_imx882_eeprom = {
	.name = "mot_imx882_eeprom",
	.sensor_type = MAIN_CAMERA,
	.check_layout_function = mot_layout_no_ck,
	.read_function = Common_read_region,
	.mot_do_factory_verify_function = imx882_mot_do_factory_verify,
	...
	.enable_preload = 1,
	.preload_size = IMX882_MOT_EEPROM_DATA_SIZE,
};
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, IMX882_PARTIAL_PD_PROC1_ADDR,
	                           IMX882_PARTIAL_PD_SIZE + 4, (unsigned char *)tempBuf);
	if (read_data_size <= 0) {
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, IMX882_PARTIAL_PD_PROC1_ADDR,
	                           IMX882_PARTIAL_PD_SIZE, (unsigned char *)&pCamCalData->PDAF.Data[0]);
	if (read_data_size <= 0) {
		err = CAM_CAL_ERR_NO_PDAF;
		return err;
	}
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, IMX882_FULL_PD_PROC1_ADDR,
	                           IMX882_FULL_PD_SIZE + 4, (unsigned char *)tempBuf);
	if (read_data_size <= 0) {
		err = CAM_CAL_ERR_NO_PDAF;
		return err;
	}
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID, IMX882_FULL_PD_PROC1_ADDR,
	                           IMX882_FULL_PD_SIZE,
	                           (unsigned char *)&pCamCalData->PDAF.Data[IMX882_PARTIAL_PD_SIZE]);
	if (read_data_size <= 0) {
		err = CAM_CAL_ERR_NO_PDAF;
		return err;
	}

	pCamCalData->PDAF.Size_of_PDAF = IMX882_PARTIAL_PD_SIZE  + IMX882_FULL_PD_SIZE;
	...
	debug_log("First five of full pd: %x, %x, %x, %x, %x\n",
		pCamCalData->PDAF.Data[IMX882_PARTIAL_PD_SIZE],
		pCamCalData->PDAF.Data[IMX882_PARTIAL_PD_SIZE+1],
		pCamCalData->PDAF.Data[IMX882_PARTIAL_PD_SIZE+2],
		pCamCalData->PDAF.Data[IMX882_PARTIAL_PD_SIZE+3],
		pCamCalData->PDAF.Data[IMX882_PARTIAL_PD_SIZE+4]);
...
unsigned int mot_imx882_do_2a_gain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	struct STRUCT_CAM_CAL_DATA_STRUCT *pCamCalData =
				(struct STRUCT_CAM_CAL_DATA_STRUCT *)pGetSensorCalData;
	...
	int af_addr = IMX882_MOT_AF_ADDR;
	int af_size = IMX882_MOT_AF_DATA_SIZE;
	int awb_addr = IMX882_MOT_AWB_ADDR;
	int awb_size = IMX882_MOT_AWB_DATA_SIZE;
...
unsigned int imx882_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData)
{
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			IMX882_MOT_EEPROM_ADDR, IMX882_MOT_EEPROM_DATA_SIZE, (unsigned char *)pCamCalData->DumpAllEepromData);
	if (read_data_size <= 0) {
		return CAM_CAL_ERR_NO_DEVICE;
	}
	//mnf check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_MOT_MNF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_MNF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData, IMX882_MOT_MNF_DATA_SIZE, checkSum)) {
	...
	memcpy(pCamCalData->serial_number, &pCamCalData->DumpAllEepromData[IMX882_MOT_SERIAL_NUMBER_ADDR],
		pCamCalData->serial_number_bit);

	//af check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_MOT_AF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_AF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_MOT_AF_ADDR, IMX882_MOT_AF_DATA_SIZE, checkSum)) {
	...
	//awb check
	if((mot_check_awb_data(pCamCalData->DumpAllEepromData+IMX882_MOT_AWB_ADDR, IMX882_MOT_AWB_DATA_SIZE + 2)) == NO_ERRORS) {
	...
	//lsc check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_MOT_LSC_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_LSC_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_MOT_LSC_ADDR, IMX882_MOT_LSC_DATA_SIZE, checkSum)) {
	...
	//pdaf check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_MOT_PDAF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_PDAF_CHECKSUM_ADDR+1]);
	checkSum1 = (pCamCalData->DumpAllEepromData[IMX882_MOT_PDAF_CHECKSUM_ADDR+2])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_PDAF_CHECKSUM_ADDR+3]);
	debug_log("checkSum  = 0x%x, checkSum1 = 0x%x", checkSum, checkSum1);

	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_MOT_PDAF1_ADDR, IMX882_MOT_PDAF_DATA1_SIZE, checkSum)
		&& check_crc16(pCamCalData->DumpAllEepromData+IMX882_MOT_PDAF2_ADDR, IMX882_MOT_PDAF_DATA2_SIZE, checkSum1)) {
	...
	//af sync data check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_MOT_AF_SYNC_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_AF_SYNC_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_MOT_AF_SYNC_DATA_ADDR, IMX882_MOT_AF_SYNC_DATA_SIZE, checkSum)) {
	...
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_MOT_MTK_NECESSARY_DATA_ADDR, IMX882_MOT_MTK_NECESSARY_DATA_SIZE, checkSum)) {
	...
	//imx882 qsc data check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_QSC_CRC_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_QSC_CRC_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_QSC_DATA_ADDR, IMX882_QSC_DATA_SIZE, checkSum)) {
	...
	//imx882 spc data check
	checkSum = (pCamCalData->DumpAllEepromData[IMX882_SPC_CRC_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[IMX882_SPC_CRC_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+IMX882_SPC_DATA_ADDR, IMX882_SPC_DATA_SIZE, checkSum)) {
